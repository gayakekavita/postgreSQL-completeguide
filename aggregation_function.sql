...
-- Above returns user_id with duplication.


select user_id from comments GROUP BY user_id;
--Above query returns user_id with unique values.
...
--It gives error because we are applying GROUP BY clause on user_id and we are trying to access the another column so it gives error here.
...
-- All above queries returns counts of specified column records in the specified column.
